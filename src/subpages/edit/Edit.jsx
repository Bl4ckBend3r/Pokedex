...
const Edit = () => {
  ...
  const onSubmit = async (data) => {
    const pokemon = pokemons.find(p => String(p.id) === String(editId));
    ...
    const res = await fetch(`http://localhost:3000/pokemons/${payload.id}`);
    const existsInDb = res.ok;
  
    if (existsInDb) {
      await fetch(`http://localhost:3000/pokemons/${payload.id}`, {
        method: "PATCH",
